...
class Admins::UserControllerTest < Test::Unit::TestCase
  ...
  def test_list
    ...
    assert_equal(users(:mr_fancy_pants_user).id,assigns(:users)[2].id)
    assert_equal(users(:peter_user).id,assigns(:users)[3].id)
    assert_equal(users(:sleepy_user).id,assigns(:users)[4].id)
  ...
  def test_list_with_query
    ...
    assert_equal(users(:peter_user).id,assigns(:users)[0].id)
    assert_nil(flash[:alert])
    assert_nil(flash[:notice])
  end
  ...
  def test_toggle_admin_status_to_true
    assert(!users(:peter_user).administrator?)
    post(:toggle_admin_status,
        {:id => users(:peter_user).id, :admin_status => 'true', :q => 's', :page => '1'},
        {:user_id => users(:admin_user).id, :role => :administrator} )
    assert_redirected_to(:action => 'list', :q => 's', :page => '1')
    assert_not_nil(assigns(:user))
    assert_equal(users(:peter_user).id,assigns(:user).id)
  ...
  def test_toggle_admin_status_to_false
    ...
    post(:toggle_admin_status,
        {:id => users(:admin_user).id, :admin_status => 'false', :q => 's', :page => '1'},
        {:user_id => users(:admin_user).id, :role => :administrator} )
    assert_redirected_to(:action => 'list', :q => 's', :page => '1')
    assert_not_nil(assigns(:user))
    assert_equal(users(:admin_user).id,assigns(:user).id)
    assert_nil(flash[:alert])
    assert_equal('Admin status successfully updated for User.',flash[:notice])
    assert(!assigns(:user).administrator?)
  end
  ...
  def test_destroy
    assert(User.exists?(users(:peter_user).id))
    post(:destroy,
